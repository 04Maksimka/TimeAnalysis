...
```powershell
uv pip install -r trading/freqtrade/requirements-hyperopt.txt
```

`optuna` was not installed initially, so the Hyperopt optional dependencies were
installed into the current `.venv`.
